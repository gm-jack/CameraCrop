...
import com.example.cameracrop.CameraCallback;
...
public class CameraUtils {
    ...
    public static final int DEFAULT_WIDTH = 3264;
    public static final int DEFAULT_HEIGHT = 1836;
    public static final int DESIRED_PREVIEW_FPS = 40;
    ...
    private static Camera.AutoFocusCallback mFocusCallBack = new Camera.AutoFocusCallback() {
        @Override
        public void onAutoFocus(boolean success, Camera camera) {
            isLock = false;
        }
    };
    private static boolean isSafeTakePicture = true;
    ...
    public static void openFrontalCamera(int expectFps) {
        ...
        if (parameters.getFlashMode().contains(Camera.Parameters.FLASH_MODE_AUTO))
        ...
        setPreviewOritation(mOrientation);
//        Logger.e("像素  " + getPixels());
    }
     ...
     * @return
     */
    public static boolean getFocusMode() {
        return mCamera.getParameters().getSupportedFocusModes().contains(Camera.Parameters.FOCUS_MODE_CONTINUOUS_PICTURE);
    }
     ...
     * @param oritation
     */
    public static void setPreviewOritation(int oritation) {
        if (mCamera != null)
            mCamera.setDisplayOrientation(oritation);
    }
    ...
    public static void setFlashMode(boolean open) {
        if (mCamera != null) {
            Camera.Parameters parameters = mCamera.getParameters();
     ...
     * @param holder
     */
    public static void startPreviewDisplay(SurfaceHolder holder, CameraCallback callback) {
        if (mCamera == null) {
        ...
        try {
            if (callback != null) {
                ...
                byte[] bytes = new byte[previewSize.width * previewSize.height * ImageFormat.getBitsPerPixel(ImageFormat.NV21) / 8];
                mCamera.addCallbackBuffer(bytes);
                mCamera.setPreviewCallbackWithBuffer(new Camera.PreviewCallback() {
                    @Override
                    public void onPreviewFrame(byte[] data, Camera camera) {
                        callback.callback(bytes, camera);
                    }
                });
            }
     ...
     * @param cameraID
     */
    public static void switchCamera(int cameraID, SurfaceHolder holder, CameraCallback callback) {
     ...
     * 预览回调打开闪光灯
     */
    public static void startPreviewFlash(boolean isOpen) {
        if (mCamera != null) {
            setFlashMode(isOpen);
            mCamera.startPreview();
        }
     ...
     * 预览回调打开闪光灯
     */
    public static void stopPreviewFlash(boolean isOpen) {
        if (mCamera != null) {
            setFlashMode(isOpen);
            mCamera.startPreview();
        }
     ...
     * @param isLockCa
     */
    public static void setIsLock(boolean isLockCa) {
        isLock = isLockCa;
    }
    ...
    public static void takePicture(Camera.ShutterCallback shutterCallback,
        ...
        if (mCamera != null && isSafeTakePicture) {
            isSafeTakePicture = false;
            mCamera.takePicture(shutterCallback, rawCallback, new Camera.PictureCallback() {
                ...
                public void onPictureTaken(byte[] data, Camera camera) {
                    ...
                    isSafeTakePicture = true;
                }
            });
        }
    }

    public static void setIsSafeTakePicture(boolean isSafe) {
        isSafeTakePicture = isSafe;
    }

    public static boolean isIsSafeTakePicture() {
        return isSafeTakePicture;
    }
    ...
    public static int getPixels() {
        ...
        sortList(supportedPictureSizes); // 根据宽度进行排序
        Camera.Size size = supportedPictureSizes.get(supportedPictureSizes.size() - 1);
        return size.width * size.height / 10000;
    }
